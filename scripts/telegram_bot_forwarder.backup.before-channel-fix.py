...
# Non-VERBOSE, DOTALL+IGNORECASE (so # is not a comment).
BID_RX = re.compile(
...
def render_dashboard() -> Panel:
    ...
    hdr.add_row(
        ...
        Text("CONNECTED", style="good"),
    )
    hdr.add_row(
        Text("Source", style="muted"),
        Text(str(SOURCE_CHANNEL_ID), style="accent"),
    )
    hdr.add_row(
        Text("Target", style="muted"),
        Text(str(TARGET_GROUP_ID), style="accent"),
    )
...
async def ui_loop(stop_event: asyncio.Event):
    with Live(render_dashboard(), console=console, refresh_per_second=8, screen=True):
        while not stop_event.is_set():
            ...
            # Just re-render by printing; Live handles it
...
async def on_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with state_lock:
            forwarded_count += 1
        push_event(f"Forwarded message {msg.message_id} to target.")
        log.info("Forwarded %s from %s -> %s", msg.message_id, SOURCE_CHANNEL_ID, TARGET_GROUP_ID)
    except Exception as e:
        push_event(f"[error]Forward failed: {e}[/error]")
    ...
    if not parsed:
        push_event("Message did not match bid pattern; forwarded only.")
...
async def run_bot(stop_event: asyncio.Event):
    app = build_app()
    # run_polling will keep going until SIGINT/SIGTERM triggers stop_event
