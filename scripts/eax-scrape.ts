...
async function main() {
  ...
  for (const r of rows) {
    ...
    await dbQuery`
      ...
      values (
        'EAX', ${r.rr_number}, ${r.tm_number}, coalesce(${r.status_code},'OPEN'), ${r.status_code},
        ${r.pickup_date}::date, ${r.pickup_window},
        ${r.delivery_date}::date, ${r.delivery_window},
        ${r.equipment}, ${r.total_miles}, null,
        ...
        ${r.vendor_name}, ${r.dispatcher_name}, now(), now()
      )
