...
BID_RX = re.compile(
    rf"""
    ^\s*New\ Load\ Bid:\s*(?P<bid>\d+)\s*[\r\n]+
    Distance:\s*(?P<dist>[\d\.]+)\s*mi(?:les)?\s*[\r\n]+
    Pickup:\s*(?P<pick>{DT_RX})\s*[\r\n]+
    Delivery:\s*(?P<deliv>{DT_RX})\s*[\r\n]+
    [^\S\r\n]*(?:[\U0001F69B🚛])?Stops:\s*[\r\n]+
    (?P<stops>(?:\s*Stop\s*\d+:\s*.*\n?)+)
    (?:\s*\n\s*(?P<tag>#[A-Za-z0-9_]+))?
    """,
    re.IGNORECASE | re.VERBOSE | re.UNICODE,
)

STOP_RX = re.compile(r"Stop\s*\d+:\s*(?P<place>.+)", re.IGNORECASE)

def _parse_dt_text(dt_text: str) -> Optional[datetime]:
    m = re.search(DT_RX, dt_text or "")
    if not m:
        return None
    ...
    if ampm.upper() == "PM":
        hh += 12
    # Interpret as UTC (adjust here if you need a specific TZ)
...
async def ui_loop():
    while True:
        ...
        if last_bid_at:
            since = humanize.naturaldelta(now - last_bid_at)
        ...
        print(f"│ Since Last Bid               {since} ({since_clock}){'':<21}│")
        print(f"│ 30-min Countdown             {countdown:<43}│")
        print("╰──────────────────────────────────────────────────────────────────────────────╯")
        await asyncio.sleep(1)

# ---------- Telegram Handlers ----------
def _get_effective_message(update: Update) -> Optional[Message]:
    # Works for channel_post, message, edited variants
    return update.effective_message

async def on_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global forwarded_count, parsed_count, last_bid_at, last_bid_number, last_tag

    msg = _get_effective_message(update)
    if not msg:
        return

    # Only react to our source channel id
    if msg.chat_id != SOURCE_CHANNEL_ID:
        return

    # 1) Forward (copy) into your private group
    try:
    ...
    # 2) Parse & Upsert if looks like a “New Load Bid”
...
def build_app() -> Application:
    # Single WIDE handler: we’ll filter by chat id in code (most reliable)
    return (
        ApplicationBuilder()
        .token(BOT_TOKEN)
        .build()
        .add_handler(MessageHandler(filters.ALL, on_any_message))
    )
