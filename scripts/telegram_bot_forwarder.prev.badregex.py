...
# Keep the pattern tolerant:
# - Allow any spacing/emoji lines between sections
# - Match 'Stops:' literally (no emoji requirement)
BID_RX = re.compile(
    rf"""
    ^\s*New\s+Load\s+Bid:\s*(?P<bid>\d+)\s*[\r\n]+
    Distance:\s*(?P<dist>[\d\.]+)\s*mi(?:les)?\s*[\r\n]+
    Pickup:\s*(?P<pick>.+)\s*[\r\n]+
    Delivery:\s*(?P<deliv>.+)\s*[\r\n]+
    (?:.*\n)*?                           # tolerate extra/emoji lines
    Stops:\s*[\r\n]+
