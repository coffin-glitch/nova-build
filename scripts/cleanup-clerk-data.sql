...
-- 5. Delete all assignments with only clerk user_id
...
-- Delete assignments with only user_id (Clerk)
...
-- Delete message reads with only Clerk user_id
...
-- Note: We're keeping user_roles_cache records even if they have clerk_user_id
-- because they might be used for migration reference. You can delete them separately if needed:
-- DELETE FROM user_roles_cache WHERE supabase_user_id IS NULL AND clerk_user_id IS NOT NULL;
...
UNION ALL
SELECT 'assignments', COUNT(*) FROM assignments WHERE supabase_user_id IS NULL AND user_id IS NOT NULL
UNION ALL
